<div>
    @if (Gate::check('addTeamMember', $company))
                    ...
                    <x-filament::card>
                        ...
                        @if (count($this->roles) > 0)
                            <div class="col-span-6 lg:col-span-4">
                                <x-forms::field-wrapper id="role" statePath="role" required="true"
                                    label="{{ __('filament-companies::default.labels.role') }}">
                                        ...
                                        @foreach ($this->roles as $index => $role)
                                            <button type="button"
                                                ...
                                                <div
                                                    ...
                                                    <!-- Role Description -->
                                                    <div
                                                        class="mt-2 text-left text-xs text-gray-600 dark:text-gray-400">
                                                        {{ $role->description }}
                                                    </div>
                                                </div>
                                            </button>
    ...
    @if ($company->teamInvitations->isNotEmpty() && Gate::check('addTeamMember', $company))
        <x-filament-companies::section-border />
        ...
        <x-filament-companies::action-section>
            ...
            <x-slot name="content">
                <x-filament::card>
                    ...
                    @foreach ($company->teamInvitations as $invitation)
                        <x-filament::hr />
                    ...
                    @foreach ($company->users->sortBy('name') as $user)
                        ...
                        <div class="flex items-center justify-between">
                            ...
                            <div class="flex items-center">
                                ...
                                @if (Gate::check('addTeamMember', $company) && Wallo\FilamentCompanies\FilamentCompanies::hasRoles())
                                    <x-filament::button class="ml-2 mr-2"
                                        wire:click="manageRole('{{ $user->id }}')">
                                        {{ Wallo\FilamentCompanies\FilamentCompanies::findRole($user->membership->role)->name }}
                                    </x-filament::button>
